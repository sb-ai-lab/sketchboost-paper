...
def get_tabnet(baseline_dict, dataset, stats):
    d = {}
    baseline_dict = baseline_dict[(dataset, 'tabnet')]

    for stat in stats:
        arr = baseline_dict[stat]
        d[stat] = np.mean(arr)
        d['std_' + stat] = np.std(arr, ddof=1)
...
if __name__ == '__main__':
    ...
    baselines = joblib.load('runs/baselines_and_params.pkl')
    ...
    for data in dataset_order:
        ...
        sub = get_stat(fold)
        res['pb'] = sub
        res['tabnet'] = get_tabnet(baselines, data, stat_order)
